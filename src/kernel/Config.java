...
public class Config {
    ...
    private static void load(String name) {
        ...
        try { fileInputStream = new FileInputStream(name);
            ...
            kernel.Console.instance.write(" > Config : file not found !");
            verify(name);
            return;
        }
        try { properties.load(fileInputStream);
        } catch(Exception e) {
            kernel.Console.instance.write(" > Config : cannot be load the file !");
            verify(name);
            return;
        }
        try { fileInputStream.close();
        } catch (IOException e) {
            kernel.Console.instance.write(" > Config : cannot be close the file !");
            verify(name);
            return;
        }
        ...
        try {
            ...
            kernel.Console.instance.write(" > Config : not found or invalid parameters! (line " + (i == 0 ? "undefinded" : i) + ")");
            verify(name);
            return;
        }

        kernel.Console.instance.write(" > Config : config loaded with success !");
    }

    private static void create(String name) {
        kernel.Console.instance.write(" > Config : the configuration file don't exist !");
        BufferedWriter config;

        try { config = new BufferedWriter(new FileWriter(name, true));
        } catch (IOException e) {
            kernel.Console.instance.write(" > Config : error on creating !");
            verify(name);
            return;
        }

        kernel.Console.instance.write(" > Config : the configuration file was created with successful !");
        write(name, config);
    }

    private static void write(String name, BufferedWriter config) {
        if(config == null) {
            kernel.Console.instance.write(" > Config : config is null !");
            verify(name);
            return;
        }
        ...
        try {
            ...
            kernel.Console.instance.write(" > Config : error on writing !");
            verify(name);
            return;
        }
    ...
    private static void waiting(String name) {
        for(int i = 0; i < 10; i++) {
            kernel.Console.instance.write(" > Config : please, set the config properties, reloading into " + (10 - i) + "s.");
